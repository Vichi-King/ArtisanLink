...
UPLOAD_FOLDER = os.path.join("app", "static", "uploads", "artisans")
...
def register_routes(app):
    @app.route("/")
    def home():
        return render_template("home.html", featured_artisans=ranked_artisans(limit=4))
    ...
    def register():
        ...
        if request.method == "POST":
            ...
            if role == "artisan":
                ...
                if not all([full_name, email, phone, category, experience_value, location, description]):
            ...
            if role == "artisan":
                ...
                if image and image.filename:
                    ...
                    profile_picture = f"{uuid.uuid4().hex}.{extension}"
                    image.save(os.path.join(UPLOAD_FOLDER, profile_picture))
    ...
    def become_artisan():
        ...
        if request.method == "POST":
            ...
            try:
                ...
                return redirect(url_for("become_artisan"))
            user.role = "artisan"
            db.session.add(Artisan(user_id=user.id, bio=bio, phone=phone, location=location, category=category, experience=experience))
            db.session.commit()
            session["user_role"] = "artisan"
            flash("Congratulations! You are now registered as an artisan.", "success")
            return redirect(url_for("artisan_dashboard"))
    ...
    def artisan_dashboard():
        ...
        return render_template("artisan_dashboard.html", user=user, artisan=artisan)
    ...
    def edit_artisan_profile():
        ...
        return render_template("edit_artisan_profile.html", user=user, artisan=artisan)
    ...
    def artisan_profile(artisan_id):
        ...
        reviews = Review.query.filter_by(artisan_id=artisan.id).order_by(Review.id.desc()).limit(5).all()
        return render_template("artisan_public_profile.html", artisan=artisan, rating=float(rating or 0), review_count=review_count, completed_jobs=completed_jobs, reviews=reviews)

    @app.route("/artisan/services/add", methods=["GET", "POST"])
    @role_required("artisan")
    def add_service():
        artisan = User.query.get(session["user_id"]).artisan_profile
        if not artisan:
            flash("Your artisan profile could not be found.", "danger")
            return redirect(url_for("artisan_dashboard"))
        if request.method == "POST":
            ...
            try:
                ...
                return redirect(url_for("add_service"))
            db.session.add(Service(artisan_id=artisan.id, name=name, category=category, description=description or None, price=price))
            db.session.commit()
    ...
    def edit_service(service_id):
        ...
        if request.method == "POST":
            ...
            try:
                ...
                flash("Price must be a positive number.", "danger")
                return redirect(url_for("edit_service", service_id=service_id))
            service.name, service.category, service.description, service.price = name, category, description or None, price
            db.session.commit()
    ...
    def delete_service(service_id):
        ...
        if not artisan or service.artisan_id != artisan.id:
            flash("You can only manage your own services.", "danger")
            return redirect(url_for("artisan_dashboard"))
        if Booking.query.filter_by(service_id=service.id).first():
    ...
    def customer_dashboard():
        user = User.query.get(session["user_id"])
        recent_bookings = Booking.query.filter_by(customer_id=user.id).order_by(Booking.id.desc()).limit(5).all()
        ...
        return render_template("customer_dashboard.html", user=user, recent_bookings=recent_bookings, booking_total=booking_total, pending_total=pending_total, completed_total=completed_total, suggested_artisans=suggested_artisans)
    ...
    def delete_account():
        ...
        Review.query.filter_by(customer_id=user.id).delete(synchronize_session=False)
        Review.query.filter_by(artisan_id=artisan.id).delete(synchronize_session=False) if artisan else None
        Booking.query.filter_by(customer_id=user.id).delete(synchronize_session=False)
        if artisan:
            Booking.query.filter_by(artisan_id=artisan.id).delete(synchronize_session=False)
        ...
        session.clear()
        flash("Your ArtisanLink account has been permanently deleted.", "info")
        return redirect(url_for("home"))
    ...
    def book_service(artisan_id, service_id):
        ...
        if request.method == "POST":
            ...
            if booking_date < datetime.now():
                ...
                return redirect(url_for("book_service", artisan_id=artisan_id, service_id=service_id))
            db.session.add(Booking(customer_id=session["user_id"], artisan_id=artisan_id, service_id=service_id, booking_date=booking_date, status="pending"))
            db.session.commit()
    ...
    @role_required("customer")
    def my_bookings():
        bookings = Booking.query.filter_by(customer_id=session["user_id"]).order_by(Booking.booking_date.desc()).all()
    ...
    def artisan_bookings():
        artisan = User.query.get(session["user_id"]).artisan_profile
        bookings = Booking.query.filter_by(artisan_id=artisan.id).order_by(Booking.booking_date.desc()).all() if artisan else []
    ...
    def respond_booking(booking_id):
        ...
        valid_transitions = {
            ...
            "complete": ("accepted", "awaiting_customer_confirmation"),
        }
        ...
        if new_status == "awaiting_customer_confirmation":
            flash("The customer has been asked to confirm whether the work was completed.", "success")
        else:
            flash(f"Booking marked as {new_status}.", "success")
        return redirect(url_for("artisan_bookings"))

    @app.route("/bookings/<int:booking_id>/completion", methods=["POST"])
    @role_required("customer")
    def confirm_completion(booking_id):
        booking = Booking.query.get_or_404(booking_id)
        if booking.customer_id != session["user_id"]:
            flash("You can only update your own bookings.", "danger")
            return redirect(url_for("my_bookings"))
        if booking.status != "awaiting_customer_confirmation":
            flash("This booking is not waiting for completion confirmation.", "warning")
            return redirect(url_for("my_bookings"))
        action = request.form.get("action")
        if action == "completed":
            booking.status = "completed"
            db.session.commit()
            flash("Thanks for confirming. You can now leave a review for the artisan.", "success")
        elif action == "not_completed":
            booking.status = "not_completed"
            db.session.commit()
            flash("Thanks for letting us know. The booking has been recorded as not completed.", "warning")
        else:
            flash("Please choose whether the work was completed.", "danger")
        return redirect(url_for("my_bookings"))

    @app.route("/bookings/<int:booking_id>/review", methods=["POST"])
    @role_required("customer")
    def submit_review(booking_id):
        ...
        if booking.status != "completed":
            flash("A review can only be submitted after you confirm the work was completed.", "warning")
            return redirect(url_for("my_bookings"))
        ...
        try:
            ...
            return redirect(url_for("my_bookings"))
        existing_review = Review.query.filter_by(customer_id=booking.customer_id, artisan_id=booking.artisan_id).first()
        if existing_review:
            flash("You have already reviewed this artisan.", "warning")
            return redirect(url_for("my_bookings"))
        db.session.add(Review(customer_id=booking.customer_id, artisan_id=booking.artisan_id, rating=rating, comment=comment or None))
        db.session.commit()
        flash("Thank you for reviewing the artisan.", "success")
        return redirect(url_for("my_bookings"))
    ...
    def admin_dashboard():
        ...
        booking_counts = dict(db.session.query(Booking.status, func.count(Booking.id)).group_by(Booking.status).all())
        completed_revenue = db.session.query(func.coalesce(func.sum(Service.price), 0)).join(Booking, Booking.service_id == Service.id).filter(Booking.status == "completed").scalar()
        average_rating = db.session.query(func.coalesce(func.avg(Review.rating), 0)).scalar()
        all_artisans = Artisan.query.join(User).order_by(Artisan.is_verified.asc(), Artisan.id.desc()).all()
        recent_bookings = Booking.query.order_by(Booking.id.desc()).limit(8).all()
        return render_template("admin_dashboard.html", total_customers=total_customers, total_artisans=total_artisans, verified_artisans=verified_artisans, unverified_count=total_artisans - verified_artisans, booking_counts=booking_counts, completed_revenue=completed_revenue, average_rating=float(average_rating or 0), all_artisans=all_artisans, recent_bookings=recent_bookings)
    ...
    def remove_artisan(artisan_id):
        ...
        user = artisan.user
        name = user.full_name
        Review.query.filter_by(artisan_id=artisan.id).delete(synchronize_session=False)
        Booking.query.filter_by(artisan_id=artisan.id).delete(synchronize_session=False)
        db.session.delete(user)
        db.session.commit()
    ...
    def contact():
        if request.method == "POST":
            ...
            flash("Your message has been received. Thank you for contacting ArtisanLink.", "success")
