...
def register_booking_routes(app):
    ...
    def negotiate_booking(booking_id):
        ...
        if request.method == "POST":
            ...
            db.session.add(BookingProposal(booking_id=booking.id, proposed_by="artisan", proposed_price=proposed_price, proposed_date=proposed_date, message=message or None, status="pending"))
    ...
    @app.route("/bookings/<int:booking_id>/negotiate", methods=["GET"])
    def customer_negotiate_booking(booking_id):
        if session.get("user_role") != "customer":
            flash("Only the customer can negotiate this booking.", "danger")
            return redirect(url_for("home"))
        booking = Booking.query.get_or_404(booking_id)
        ...
        if booking.status not in {"pending", "negotiating"}:
            flash("This booking is no longer open for negotiation.", "warning")
            return redirect(url_for("my_bookings"))
        return render_template("booking_negotiation.html", booking=booking, role="customer")
    ...
    def customer_counter_proposal(booking_id):
        ...
        if proposed_price is None or proposed_date is None or proposed_date < datetime.now():
            ...
            return redirect(url_for("customer_negotiate_booking", booking_id=booking.id))
        _reject_pending_proposals(booking)
        db.session.add(BookingProposal(booking_id=booking.id, proposed_by="customer", proposed_price=proposed_price, proposed_date=proposed_date, message=message or None, status="pending"))
        booking.status = "negotiating"
        db.session.commit()
